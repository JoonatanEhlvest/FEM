...
describe("XMLEditor Tests", () => {
	...
	test("Update instance description", async () => {
		...
		if (result.success && result.xml) {
			fs.writeFileSync(
				path.resolve(tempDir, "updated-description.xml"),
				result.xml
			);
			...
			const updatedEditor = createXMLEditor(result.xml, {
				preserveDoctype: true,
			});
			...
			if (updatedInstance) {
				...
				const descriptionAttr = attributeArray.find(
