...
import { getInstanceDisplayProperties, getInstanceStyle, InstanceDisplayStyle } from '../utils/instanceMap';
  ...
  zoom
    ...
    switch (displayProperties.shape.type) {
        ...
        return renderRectangle(
            ...
            height, 
            style, 
            displayProperties.shape.cornerRadius || SHAPE_SETTINGS.CORNER_RADIUS
        );
    }
    };

    // Function to get custom fill color for the instance
    function getCustomFillColor(instance: Instance, model: Model): string | undefined {
    ...
    if (instance.Interrefs && instance.Interrefs["Referenced Subclass"] && 
        (instance.colorPicker === "Subclass" || instance.colorPicker === undefined)) {
    ...
    if ((instance.colorPicker === "Individual" || instance.colorPicker === undefined)) {
        if (instance.isGhost && instance.individualGhostBGColor) {
        const color = instance.individualGhostBGColor.replace('$', '#');
        return color;
        } else if (instance.individualBGColor) {
    ...
    for (let i = Math.floor(name.length / 3); i <= Math.floor(name.length * 2 / 3); i++) {
    ...
    >
      ...
      {nameLines.length === 1 ? (
        <text
          ...
          fontSize={adjustedFontSize(FONT_SETTINGS.INSTANCE_NAME.fontSize)}
