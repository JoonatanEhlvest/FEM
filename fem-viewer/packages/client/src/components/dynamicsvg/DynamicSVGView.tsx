...
import InstanceRenderer, { CM_TO_PX } from './renderers/InstanceRenderer';
...
const DynamicSVGView: React.FC = () => {
  ...
  const { 
    ...
    zoomIn, 
    zoomOut 
  } = useSVGZoom(svgRef, setZoom);

  // Calculate dimensions from model's worldArea if available
  useEffect(() => {
    if (model?.attributes.worldArea && dimensions.width === 1000 && dimensions.height === 800) {
    ...
    <div className={styles["dynamicsvg-container"]}>
      ...
      <div className={styles["svg-container"]} ref={containerRef}>
        ...
        <div className={styles["zoom-controls"]}>
          <button className={styles["zoom-button"]} onClick={zoomIn}>+</button>
          <button className={styles["zoom-button"]} onClick={resetZoom}>Reset</button>
          <button className={styles["zoom-button"]} onClick={zoomOut}>-</button>
          <div className={styles["zoom-level"]}>{Math.round(zoom * 100)}%</div>
      </div>
      </div>
    </div>
  );
